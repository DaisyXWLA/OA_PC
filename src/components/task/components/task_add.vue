<template>
  <Modal v-model="isAddShow" v-bind="params">
    ...
    <div>
      ...
      >
        ...
        <FormItem label="任务描述" prop="taskContent" single>
          <!-- <RichTextEditor v-model="taskModel.taskContent"></RichTextEditor> -->
          <!-- <vue-ueditor-wrap v-model="taskModel.taskContent"></vue-ueditor-wrap> -->
    ...
    <div slot="footer">
      <Button @click="submit" class="h-btn-blue">保存</Button>
...
export default {
  // props: ['isAddShow', 'title', 'projectOption', 'member', 'taskModel'],
  props: ['isAddShow', 'title', 'projectOption', 'member'],
  components: {
    uploadFile
  },
  data() {
    return {
      taskModel: taskList.parse({ projectInfo: this.projectInfo }),
      value8: '1',
  ...
  methods: {
    ...
    submit() {
      this.taskModel.projectInfo = this.projectInfo;
      ...
      if (validResult.result) {
        ...
        R.ProjectTask.taskAdd(taskList.dispose(this.taskModel)).then(resp => {
          if (resp.ok) {
            if (this.value8 == '1') {
              setTimeout(() => {
                this.$Loading.close();
                this.$Message.success('保存成功');
                this.xiangmu = this.taskModel.projectInfo;
                this.taskModel = taskList.parse({});
                this.taskModel.priority = '0';
                this.taskModel.projectInfo = this.xiangmu;
                this.$parent.getData();
              }, 1000);
            } else {
              setTimeout(() => {
...
.h-form.h-form-single .list-task {
  overflow: hidden;
}
.h-form.h-form-single .list-task .h-form-item {
  width: 48%;
  margin-right: 4%;
  float: left;
}
.h-form.h-form-single .list-task .h-form-item:nth-child(2) {
  width: 48%;
  margin-right: 0;
  float: right;
}
