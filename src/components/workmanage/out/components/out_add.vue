<template>
  <Modal v-model="a" v-bind="params">
    <div slot="header" class="title">{{title}}<i @click="modalDelete">×</i></div>
    <div>
      <Form
        :label-width="110"
        :mode="mode"
      ...
      >
        <FormItem label="外出主题：" prop="param2">
          <input type="text" v-model="data.param2" placeholder="请输入" v-wordlimit="30" />
        </FormItem>
        <FormItem label="外出地点：" prop="param3">
          <Select v-model="data.param3" dict="simple"></Select>
        </FormItem>
          <FormItem label="外出同行人：" prop="param3">
          <Select v-model="data.param3" dict="simple"></Select>
        </FormItem>
        <FormItem label="外出时间段：" prop="param6">
          <Row :space="0">
            <Cell width="20">
        ...
        <FormItem label="外出理由：" prop="taginputsData">
