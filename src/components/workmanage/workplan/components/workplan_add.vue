<template>
  <Modal v-model="AddPlan" v-bind="params">
    <div slot="header" class="title">新增工作计划<i @click="cancel">×</i></div>
    <div>
      <Form
        :label-width="110"
        :mode="mode"
      ...
      if (validResult.result) {
        R.ProjectPlan.planAdd(workplanModel.dispose(this.workplanSerch)).then(resp => {
          if (resp.ok) {
            ...
            this.workplanSerch = workplanModel.parse({});
            this.workplanSerch.content = null;
            this.data.value = {};
            this.data.date = '';
            this.cancel();
            setTimeout(() => {
